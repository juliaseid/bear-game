...
describe('Fuzzy', () => {
  ...
  test('should have a food level of 7 after 30 seconds', () => {
  ...
  test('should get very hungry if 100 seconds pass without feeding', function () {
...
describe('Game & its properties', () => {
  ...
  test('AddAnimal function adds the correct animal to the game/zoo', () => {
    myGame.addAnimal('wuzzy', 'bear');
    expect(myGame.animals[1].name).toEqual('wuzzy');
    expect(myGame.animals[1].species).toEqual('bear');
  });

  test('If user keeps animal alive for 2 minutes, comfort level increase by 50', () => {
    myGame.stillAlive();
    jest.advanceTimersByTime(120001);
    expect(myGame.animals[0].comfortLevel).toBeGreaterThan(50) && expect(myGame.animals[0].comfortLevel).toBeLessThan(90);
  });

  test('If user keeps animal alive for 2 miniutes, food level increases by 10', () => {
    myGame.stillAlive();
    jest.advanceTimersByTime(120001);
    expect(myGame.animals[0].foodLevel).toEqual(8);
  })

  test('If animal eats/tramples user, game ends', function () {
    jest.advanceTimersByTime(300001);
    myGame.animals[0].didYouGetEaten();
    myGame.endGame();
    expect(myGame.gameOver).toEqual(true);
  })
